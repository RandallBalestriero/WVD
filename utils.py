...
def generate_sinc_filterbank(f0, f1, J, N):
    ...
    freqs = np.stack([freqs * 0.9, freqs], 1)
    freqs[:, 1] -= freqs[:, 0]
    ...
    # apodize
    apod_filters = sincs * T.signal.hanning(N)
    # normalize
...
# gaussian parameters
    mut = T.Variable(0.01 *  np.random.randn(J,1).astype('float32'),
                                    name='xvar')
    ...
    cor = T.Variable(0.3 * np.random.randn(J, 1).astype('float32'), name='cor')
    sigmat = T.Variable(1 + 0.001 * np.random.randn(J, 1).astype('float32'),
                        name='yvar')
    sigmaf = T.Variable(1 + 0.001 * np.random.randn(J, 1).astype('float32'),
                        name='cor')
    ...
    cov = T.concatenate([xvar, T.tanh(cor) * coeff,
    ...
    centered = grid - T.expand_dims(T.stack([mut, muf], 1), 0)
    gaussian = T.exp(-(T.matmul(centered, cov)**2).sum(-1))
    gaussian_2d = T.reshape(gaussian/T.linalg.norm(gaussian, 2, 1), (J, N, M))
    return gaussian_2d, mut, muf, cor, sigmat, sigmaf
...
def create_transform(input, args):
    ...
    if args.option == 'melspec':
        ...
        layer = [layers.Conv1D(input_r, W=filters.real,
                               ...
                               W_shape=filters.shape, trainable_b=False)]
        ...
        layer = [
            layers.Conv1D(
                input_r,
                W=T.real(filters_norm),
                trainable_W=False,
                strides=args.hop,
                W_shape=filters_norm.shape,
                trainable_b=False)]
        layer.append(layers.Conv1D(input_r, W=T.imag(filters_norm),
                                   trainable_W=False, strides=args.hop,
                                   W_shape=filters_norm.shape,
                                   trainable_b=False))
        ...
        WVD = T.signal.wvd(input, window=args.bins * 4, L=args.L * 2,
                            hop=args.hop)
        filters, mut, muf, cor, sigmat, sigmaf = generate_gaussian_filterbank(args.J*args.Q, args.bins*2, 32)
...
def medium_model_bird(layer, deterministic, c):
    ...
    layer.append(layers.Conv2D(layer[-1], W_shape=(32, 64, 3, 3)))
    layer.append(layers.BatchNormalization(
        layer[-1], [0, 2, 3], deterministic))
    layer.append(layers.Activation(layer[-1], T.leaky_relu))
    layer.append(layers.Pool2D(layer[-1], (4, 4)))
