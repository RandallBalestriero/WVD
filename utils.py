...
def generate_learnmorlet_filterbank(N, J, Q):
    ...
    filters = T.signal.morlet(
        N, s=0.01 + T.abs(scales.reshape((-1, 1))), w=freqs.reshape((-1, 1))
    )
...
def generate_morlet_filterbank(N, J, Q):
    ...
    filters = T.signal.morlet(
        N,
        s=0.1 + scales.reshape((-1, 1)).astype("float32"),
...
def generate_gaussian_filterbank(N, M, J, f0, f1, modes=1):
    ...
    cor = T.Variable(
        0.01 * np.random.randn(J * modes).astype("float32"), name="cor"
    )

    sigma_init = np.stack(
        [freqs / 6, 1.0 + 0.01 * np.random.randn(J * modes)], 1
    )
    ...
    Id = T.eye(2)
    cov = Id * T.expand_dims((T.abs(sigma) + 0.1), 1) + T.flip(Id, 0) * (
        T.tanh(cor) * coeff
    ).reshape((-1, 1, 1))
