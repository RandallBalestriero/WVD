...
do
    ...
    do
        CPT=$((CPT+1))
        GPU=${GPUS[CPT]}                                                                                                                                                     
        screen -dmS wvd$run$LR bash -c "export CUDA_VISIBLE_DEVICES=$GPU;python mnist.py --option wvd --bins $BINS -LR $LR -L 8 --model $MODEL --run $run  -BS $BS --hop $HOP -J $J -Q $Q"

#        CPT=$((CPT+1))                                                                                                                                                       
#        GPU=${GPUS[CPT]}                                                                                                                                                     
#        screen -dmS npwvd$run$LR bash -c "export CUDA_VISIBLE_DEVICES=$GPU;python mnist.py --option npwvd --bins $BINS -LR $LR -L 8 --model $MODEL --run $run  -BS $BS --hop $HOP -J $J -Q $Q"
