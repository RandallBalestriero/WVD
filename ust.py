...
# DATASET LOADING
args.dataset = 'dyni'
wavs_train, labels_train, wavs_valid, labels_valid, wavs_test, labels_test = data_loader.load_ust()
...
# create placeholders
label = T.Placeholder((args.BS, labels_valid.shape[1]), 'int32')
...
if args.model == 'base':
    ...
    utils.small_model_bird(layer, deterministic, labels_train.shape[1])
else:
...
loss = theanoxla.losses.sigmoid_crossentropy_logits(label, layer[-1]).mean()
#accuracy = theanoxla.losses.accuracy(label, layer[-1]).mean()
print(label.shape, layer[-1].shape)
accuracy = T.equal(label, T.greater_equal(T.nan_to_num(layer[-1]), 0).astype('int')).astype('float32').mean(0)
proba = T.softmax(layer[-1])[:, 1]
var = sum([lay.variables() for lay in layer if isinstance(lay, layers.Layer)],
          [])

lr = theanoxla.schedules.PiecewiseConstant(args.LR, {50: args.LR/3,
                                                      75: args.LR/6})
...
#    REP.append(np.concatenate(r))
    ...
    PROBA.append(np.concatenate(p))
    print('FINALtest', np.mean(TEST[-1]))
    ...
    # save the file
    np.savez('save_UST_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.npz'.format(args.BS, args.option, args.J, args.Q, args.L,
