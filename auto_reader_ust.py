...
MODELS = ['wvd', 'morlet', 'melspec', 'sinc']
LRS = [0.001, 0.005, 0.0005]
RUNS = [0]
name = 'save_UST_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.npz'
BS=4
J=5
Q=8
DN='small'
...
for RUN in RUNS:
    for lr in LRS:
        for c, model in enumerate(MODELS):
            ...
            test = f['test'].mean((1,2))
            valid = f['valid'].mean((1,2))
            print(train[:5])
            T.append(test[valid.argmax()])
            QQ.append(valid.max())
            print(train.shape, test.shape, valid.shape)
            ax1.plot(valid, c='C{}'.format(c), label=model)
            ax2.plot(test, c='C{}'.format(c))
